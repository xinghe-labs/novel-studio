...
本文件是所有模式的受控正典写入协议真源。`controlled-automation.md` 保留自然化记录和暂存包字段，`short-story-mode.md` 保留唯一正文单元约束，其他文件只补充模式差异；涉及顺序、租约或 fail-closed 语义时以本文件为准。
...
4. 每章或每篇短故事必须实际调用 `$humanizer-zh`。保存调用前原稿、最终结果和 `humanization-review.json`；Skill 不可用或记录不完整时只能排队草稿。
